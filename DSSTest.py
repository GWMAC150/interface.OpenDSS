# -*- coding: utf-8 -*-
"""
Created on Fri Jan 31 09:58:59 2020
...
class DSSService(rpyc.Service):
    ...
    def exposed_subscribe(self,sub,callback):
        print("server subscribe called")
        global sim
        sim.subscribe(sub,callback)
...
# that represents the electric circuit that is being modeled
    ...
    def run(self):
        ...
        if self.filename != "":
            ...
            self.text.Command = "set mode=daily stepsize=15m number=96"
            ...
            for steps in range(originalSteps):
                ...
                for key in self.subs:
                    for sub in self.subs[key]:
                        ...
                        if sub.attr == "Powers":
                            res = self.element.Powers
                        elif sub.attr == "Voltages":
                            res = self.element.Voltages
                        #res = exec("self.element."+sub.attr)
                        #self.text.Command ="? "+ sub.obj+"."+sub.name+"."+sub.attr
                        #res = self.text.Result
...
# the x and y coordinates of the pick event (mouse click)
    ...
    def subscribe(self,sub,callback):
        print("subscription arrived: %s" % str(sub))
        self.callback = async_(callback)
        obj,name,attr = sub
        key = '%s.%s' % (obj,name)
        drop =  re.compile(r' %s'% attr)
        with self.lock:
            if key not in self.subs: self.subs[key] = []
            #self.subs[key] += [Subscribe(client=client,obj=obj,attr=attr,unit=drop)]
            self.subs[key] += [Subscribe(obj=obj,name=name,attr=attr)]
...
class DSSSimulationThread(threading.Thread):
    def __init__(self):
        threading.Thread.__init__(self)
        global d
    def run(self):
...
class DSSServerThread(threading.Thread):
    def __init__(self):
        threading.Thread.__init__(self)
        self.callback = None
        
    def run(self):
        t = ThreadedServer(DSSService, port=18862, auto_register=True,
...
if __name__ == "__main__":
    global sim
    serv = DSSServerThread()
    sim = DSS(r"C:\Program Files\OpenDSS\IEEETestCases\123Bus\IEEE123Master.dss")
    sim.start()
    serv.start()
    serv.join()
    sim.join()